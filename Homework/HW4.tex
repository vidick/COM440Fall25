...
\[ \Pr\big[ \hat{X}\in B^{\delta n}(X)\big] \leq 2^{-\frac12 (\Hmin(X|E)-1)+\log(B^{\delta n})}\;.\]
\end{enumerate}
...
\item Starting from 
\[  Q^1(L^1)=\sum_{z\in L^1} Q^1(z) \leq \sum_{z\in L^1} \big(\sqrt{p}\xi_z +2^{-n/2}\sqrt{|L^1|}\big)^2\;,\]
