...
The final has 13 short questions, to be handed in on this sheet of paper (total: 6.5pts) and 4 problems to be handed in on your own paper (total: 26pts), for a total of 5 double-sided sheets of paper. Your total out of 32.5 points will be scaled down to a grade out of 20 which will constitute your final grade. 

		
\section{Short questions}

Circle the correct answer directly on this sheet of paper, and turn it in with your full name and SCIPER number clearly marked at the top. \emph{(0.5pt per question)}
\begin{enumerate}
...
\item No
\end{enumerate}
\item Suppose $\Ext: \{0,1\}^n\times \{0,1\}^d\rightarrow \{0,1\}^m$ is a $(k,\eps)$ strong extractor. For which of the following random variables $X$ and $Y$ is it  true that $\|\rho_{\Ext(X,Y)YE} - I / 2^m \otimes \rho_{YE}\|_{tr} \leq \eps$? Circle all that apply. 
\begin{enumerate}
...
\item Consider now a scenario in which Alice's strategy consists in choosing one out of three possible unitaries, according to a distribution that may depend on her choice of $\theta$, and then measuring. The three unitaries are the same whether $\theta=0$ or $\theta=1$, and Eve knows what they are as well as the distribution used by Alice, but she does not learn which unitary Alice eventually selected. Provide a set of three unitaries that makes Eve's guessing probability the lowest possible. (You do not need to prove that your strategy is optimal.)
...
Here are three suggested protocols that Alice and Bob might want to implement. For
each of them, either argue (in broad lines) security or provide an explicit attack by which Eve can recover the key produced by Alice. (If arguing security, you will be arguing that for some ``reasonable'' choice of the unspecified parameters, such as for privacy amplification, Alice and Bob can obtain a secure key based on the template provided.)

For each protocol, Alice and Bob can communicate through a classical authenticated channel, and a
...
\begin{enumerate}
\item Alice generates bit strings $x, \theta \in\{0, 1\}^n$ uniformly at random.
\item Alice generates two-qubit states $|x_i\rangle\otimes  H\ket{\theta_i}$ for $i = 1,\ldots, n$. Here, the first qubit has $x_i$ encoded in the standard basis, while the second qubit has $\theta_i$ encoded in the Hadamard basis. 
\item Alice sends the two-qubit states to Bob. 
\item Bob announces receipt of the states.
\item Bob generates a string $\theta' \in\{0, 1\}^n$ uniformly at random. He measures the second qubits of the two-qubit states he received according to the bases specified by the string
$\theta'$ and obtains a string $\chi'$ as output.
\item Alice announces the basis string $\theta$, and Bob announces $\chi'$, over the authenticated channel.
\item  Whenver $i\in\{1,\ldots,n\}$ is such that $\theta_i=\chi'_i$, Bob measures the first qubit of the $i$th pair received from Alice in the computational basis to obtain a string $x'_i$. Let $S$ denote the set of such $i$'s.
\item Alice and Bob perform information reconciliation and privacy amplification, on the strings $x_S$ and $x'_S$ respectively, with adequate parameters to obtain a shared key. 
\end{enumerate}
...
\item Alice randomly selects a subset $T\subseteq\{i: \theta_i=\theta'_i\}$ of size $|T|=n/4$ and announces it to Bob. 
\item Alice and Bob perform information reconciliation and privacy amplification on the strings $x_T$ and $x'_T$ respectively, with adequate parameters to obtain a shared key. 
\end{enumerate}
...
For a cheating Alice and honest Bob, we define Alice's cheating probability as \[ P_A^*= \frac{1}{2}\big(\Pr[\mbox{Alice opens $b=0$ successfully}]+\Pr[\mbox{Alice opens $b=1$ successfully}]\big)\;,\]
 maximized over Alice's (cheating) strategies. For a cheating Bob and an honest Alice who commits to $b \in \{0, 1\}$ with equal probability, instead, we let Bob's cheating probability be $$P_B^* = \Pr[\mbox{Bob guesses b after the commit phase}]\;,$$ maximized over Bob's (cheating) strategies. The cheating probability of the protocol as a whole is then defined as $\max \{P_A^*,P_B^*\} - \frac{1}{2}$. In this question, we introduce a simple example of such a protocol:
\begin{itemize}
...
\begin{enumerate}
\item[i.] First, the challenger chooses a uniformly random message $m\in\{0,1\}^n$, a uniformly random key $k\in\mathcal{K}$ and sets $\rho_A\leftarrow \Enc_k(m)$. The challenger sends $\rho_A$ to Alice. 
\item[ii.] Alice applies an arbitrary quantum channel on $\rho_A$, to create a bipartite state $\sigma_{BC}\in D(\mathcal{H}_B\otimes \mH_C)$, i.e.\ Alice applies $T: D(\mathcal{H}_A)\to D(\mathcal{H}_B\otimes \mH_C)$ and sets $\sigma_{BC}\leftarrow T(\rho_A)$.
\item[iii.] Bob receives the register $B$, and Charlie receives $C$. So, the reduced density received by Bob is $\sigma_B$ and the one received by Charlie is $\sigma_C$. (Depending on the map $T$ that was applied by Alice, these two states might be entangled. For example, Alice's map $T$ could throw away $\rho_A$ and create EPR pairs between $B$ and $C$.)
...
\end{enumerate}
In addition, we modify the last step as follows.
\begin{enumerate}
...
\end{enumerate}
Because the game now involves guessing a single bit, we say that an encryption scheme is \emph{strongly} $t$-uncloneable secure if the maximum success probability of Alice, Bob and Charlie in this game is at most $\frac{1}{2}+2^{-(n-t)}$.
\begin{enumerate}
\item[(f)] Show that, if an encryption scheme has uncloneable security $t$ according to the new definition, then it has uncloneable security $t'$ according to the old definition (for some $t'$; state an upper bound on $t'$ as a function of $t$), and furthermore is $\eps$-approximately secure as an encryption scheme, for some $\eps$ depending on $t$ that you will specify. (Make sure that you state your definition of $\eps$-approximate security.)
